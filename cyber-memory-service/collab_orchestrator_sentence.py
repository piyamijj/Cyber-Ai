...
async def relay_sentence_to_usta(
    ...
    usta_sentence_system_prompt = (
        ...
        "2b. FELSEFİ/HALÜSİNASYON SAPMASI KONTROLÜ (canlı site testinde bulundu — tutarlılık için "
        "eklendi, çırak tarafında zaten davranışsal bir kural var): Aday cümle, duygusal/kurgusal/"
        "felsefi bir metafor mu içeriyor, ya da kullanıcıyla 'kardeş' gibi kişisel/insani bir ilişki "
        "kurmaya mı çalışıyor (örn. 'Sekiz milyar insanın yolculuğunun yarısını sen olduruyorum' gibi "
        "anlamsız/tutarsız bir cümle)? Bu bir yapay zeka asistanı için uygun DEĞİLDİR — eğer öyleyse "
        "ASLA SENTENCE_OK ile onaylama, bunun yerine kısa ve net bir şekilde bunun bir yapay zeka "
        "asistanı olarak yapılamayacağını belirten NESNEL bir cümleyle değiştir.\n"
...
async def run_sentence_relay_round(
    ...
    draft_system_prompt = (
        ...
        "[KATI KURAL — PAPAĞAN YASAĞI] (canlı site testinde bulunan davranışsal düzeltme — bkz. "
        "cyber-llama-draft.service.example'daki '--chat-template chatml' notu, bu kuralın "
        "TAMAMLAYICISIdır, tek başına çözüm değildir): Sen bir papağan değilsin. Kullanıcının "
        "sorduğu soruyu veya verdiği emri ASLA aynen veya hafifçe değiştirerek (örn. emir "
        "kipini soru kipine çevirerek) ona geri yazma — 'Bugünün haber başlıklarını ver' "
        "sorusuna 'Bugünün haberlerin başlıklarını belirtin.' gibi bir cevap KESİNLİKLE "
        "YASAKTIR. Kullanıcının cümlesini tekrarlamak/yeniden ifade etmek DEĞİL, SORUYA "
        "GERÇEK BİR CEVAP ÜRETMEK senin tek görevin. "
